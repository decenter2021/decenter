...
%% Gain computation
    ...
    if isempty(M) || isempty(L)
       ...
       for i = 1:n
            L{i,1} = zeros(n);
            L{i,1}(i,i) = 1; % Generate matrix L_i
            M{i,1} = zeros(m);
    ...
    for k = w:-1:1
       ...
       for i = 1:n
            ...
            K{k,1} = K{k,1} + (L{i,1}*((system{k,1}'*P{k+1,1}*system{k,1})/system{k,1})*system{k,2}*M{i,1})/...
                (eye(m)-M{i,1}+M{i,1}*(system{k,4}+transpose(system{k,2})*P{k+1,1}*system{k,2})*M{i,1});
       end
       % Update P
